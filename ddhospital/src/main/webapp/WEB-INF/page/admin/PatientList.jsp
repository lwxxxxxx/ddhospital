...
<!--修改h患者信息的表单-->
<form class="layui-form" id="editPatient" action="" lay-filter="patientInfo" style="display:none" >
...
<script>
    layui.use(['table','form'], function () {
        ...
        table.render({
            elem: '#test'
            , url: '${pageContext.request.contextPath}/pat_list'
            , cols: [[
                {field: 'id', title: '编号' }
                , {field: 'name', title: '姓名' }
                , {field: 'sex', title: '性别'}
        ...
        table.on('tool(test)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            if(layEvent === 'del'){ //删除
                layer.confirm('真的删除行么', function(index){
                    //向服务端发送删除指令
                    $.get("${pageContext.request.contextPath}/pat_delete", {"id": data.id}, function (res) {
                        if (res.code === 200) {
                ...
                //do something
                $.get("${pageContext.request.contextPath}/pat_get",{"id":data.id},function (res) {
                    if(res.code!==200){
                    ...
                    var data=res.data
                        //给表单赋值
                        form.val("patientInfo", { //formTest 即 class="layui-form" 所在元素属性 lay-filter="" 对应的值
        ...
        form.on('submit(update)', function(data){
            var data = form.val("patientInfo");
            $.post("${pageContext.request.contextPath}/pat_update",data,function (res) {
                if (res.code!=200){
                    return  layer.alert(res.message, {
                        title: '提示信息'
                    })
                }
                location.href="${pageContext.request.contextPath}/page_admin/PatientList"
